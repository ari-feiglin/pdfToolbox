...
\def\_pdfdraw_mult#1#2{\_nopt{#1\dimexpr #2pt\relax}}
\def\_pdfdraw_add#1#2{\_nopt{#1pt+#2pt}}
...
\def\_pdfdraw_ellipse#1#2#3#4{% (x,y), r1, r2
    \_pdfdraw_moveto {\_pdfdraw_add{#1}{#3}}{#2}%                                                   (x+r1, y)    m
    \_pdfdraw_bezierc{\_pdfdraw_add{#1}{#3}}{\_pdfdraw_add{#2}{\_pdfdraw_mult\_pdfdraw_cd{#4}}}%    (x+r, y+d*r2)
                    {\_pdfdraw_add{#1}{\_pdfdraw_mult\_pdfdraw_cd{#3}}}{\_pdfdraw_add{#2}{#4}}%     (x+d*r1, y+r2)
                    {#1}{\_pdfdraw_add{#2}{#4}}%                                                    (x,y+r2)     c
    \_pdfdraw_bezierc{\_pdfdraw_add{#1}{\_pdfdraw_mult{-\_pdfdraw_cd}{#3}}}{\_pdfdraw_add{#2}{#4}}% (x-d*1r,y+r2)
                    {\_pdfdraw_add{#1}{-#3}}{\_pdfdraw_add{#2}{\_pdfdraw_mult\_pdfdraw_cd{#4}}}%    (x-r1,y+d*r2)
                    {\_pdfdraw_add{#1}{-#3}}{#2}%                                                   (x-r1,y)     c
    \_pdfdraw_bezierc{\_pdfdraw_add{#1}{-#3}}{\_pdfdraw_add{#2}{\_pdfdraw_mult{-\_pdfdraw_cd}{#4}}}%(x-r1, y-d*r2)
                    {\_pdfdraw_add{#1}{\_pdfdraw_mult{-\_pdfdraw_cd}{#3}}}{\_pdfdraw_add{#2}{-#4}}% (x-d*r1, y-r2)
                    {#1}{\_pdfdraw_add{#2}{-#4}}%                                                   (x,y-r2)     c
    \_pdfdraw_bezierc{\_pdfdraw_add{#1}{\_pdfdraw_mult\_pdfdraw_cd{#3}}}{\_pdfdraw_add{#2}{-#4}}%   (x+d*r1,y-r2)
                    {\_pdfdraw_add{#1}{#3}}{\_pdfdraw_add{#2}{\_pdfdraw_mult{-\_pdfdraw_cd}{#4}}}%  (x+r1,y-d*r2)
                    {\_pdfdraw_add{#1}{#3}}{#2}%                                                    (x-r1,y)     c
}
...
\def\_pdfdraw_preaddtransform#1{%
    \_xp\_xp\_xp\def\_xp\_xp\_xp\_regA\_xp\_xp\_xp{\macrostackget{transstack}{\macrostackpointer{transstack}}}%
    \macrostackpush{transstack}\_pdfdraw_transform%
...
\def\_pdfdraw_addaffinetransform#1#2#3#4#5#6{%
    \edef\_regA##1##2{%
        {\noexpand\_pdfdraw_add{\noexpand\_pdfdraw_add{\noexpand\_pdfdraw_mult{#1}{##1}}{\noexpand\_pdfdraw_mult{#3}{##2}}}{#5}}% #1 * ##1 + #3 * ##2 + #5
        {\noexpand\_pdfdraw_add{\noexpand\_pdfdraw_add{\noexpand\_pdfdraw_mult{#2}{##1}}{\noexpand\_pdfdraw_mult{#4}{##2}}}{#6}}% #2 * ##1 + #4 * ##2 + #6
    }%
...
\def\_pdfdraw_preaddaffinetransform#1#2#3#4#5#6{%
    \edef\_regA##1##2{%
        {\noexpand\_pdfdraw_add{\noexpand\_pdfdraw_add{\noexpand\_pdfdraw_mult{#1}{##1}}{\noexpand\_pdfdraw_mult{#3}{##2}}}{#5}}% #1 * ##1 + #3 * ##2 + #5
        {\noexpand\_pdfdraw_add{\noexpand\_pdfdraw_add{\noexpand\_pdfdraw_mult{#2}{##1}}{\noexpand\_pdfdraw_mult{#4}{##2}}}{#6}}% #2 * ##1 + #4 * ##2 + #6
    }%
    \macrostackpush{affinetransstack}\_regA%
    \_xp\_xp\_xp\_pdfdraw_preaddtransform\macrostackget{affinetransstack}{\macrostackpointer{affinetransstack}}%
}
%\def\_pdfdraw_addaffinetransform#1#2#3#4{%
%    \def\_regA##1##2{%
%        {\_pdfdraw_add{\_pdfdraw_mult{#1}{##1}}{\_pdfdraw_mult{#3}{##2}}}% #1 * ##1 + #3 * ##2
%        {\_pdfdraw_add{\_pdfdraw_mult{#2}{##1}}{\_pdfdraw_mult{#4}{##2}}}% #2 * ##1 + #4 * ##2
%    }%
%    \macrostackpush{affinetransstack}\_regA%
%    \_xp\_xp\_xp\_pdfdraw_addtransform\macrostackget{affinetransstack}{\macrostackpointer{affinetransstack}}%
%}
...
%                \unexpanded{\_pdfdraw_updatedimens_list}\_varrepeat{3}{\numexpr #2+2\relax}{2}>\_pdfdraw_def_point_text\_nul\space%
%                ##2
%            Q}%
%        }%
%    }%
...
\def\_addnode#1#2#3#4{%
    \setbox0=\hbox{#3}%
    \_xp\xdef\csname point@#4.left\endcsname{\_nopt{#1pt - .5\wd0 - \coordbuf}}%
    \_xp\xdef\csname point@#4.top\endcsname{\_nopt{#2pt + .5\ht0 + .5\dp0 + \coordbuf}}%
    \_xp\xdef\csname point@#4.right\endcsname{\_nopt{#1pt + .5\wd0 + \coordbuf}}%
    \_xp\xdef\csname point@#4.bottom\endcsname{\_nopt{#2pt - .5\ht0 - .5\dp0 - \coordbuf}}%
...
%\def\drawpath#1#2#3#4{%
%    \edef\_regA{\_getcoordvalue{\_id#1}}%
%    \edef\_regB{\_getcoordvalue{\_id#2}}%
%    \edef\_regC{\_getcoordvalue{\_id#3}}%
%    \edef\_regD{\_getcoordvalue{\_id#4}}%
...
%        \_regA\space\_regB\space cm
%        0 0 m
%        \_regF\space 0 l
%        S
%        %\_pdfdraw_moveto{\_regA}{\_regB}
%        %\_pdfdraw_lineto{\_regC}{\_regD}
%        Q
%        \_multiply{\_subtract{\_regC}{\_regA}}{\_regE} \_multiply{\_subtract{\_regD}{\_regB}}{\_regE}
%        \_multiply{\_subtract{\_regB}{\_regD}}{\_regE} \_multiply{\_subtract{\_regC}{\_regA}}{\_regE}
%        \_regC\space\_regD\space cm
%        %0 0 m %-2 1 l 0 0 m -2 -1 l S
...
\_pdfdraw_addaffinetransform{0.707}{0.707}{-0.707}{0.707}{0}{0}
...
\def\drawpath#1#2#3#4{%
    ...
    \_pdfdraw_preaddaffinetransform%
        {\_multiply{\_subtract{\_regC}{\_regA}}{\_regE}}{\_multiply{\_subtract{\_regD}{\_regB}}{\_regE}}%
        {\_multiply{\_subtract{\_regB}{\_regD}}{\_regE}}{\_multiply{\_subtract{\_regC}{\_regA}}{\_regE}}%
        {\_regC}{\_regD}%
    \pdfliteral{
...
\def\definelinecap#1#2#3{%
    \_xp\edef\csname linecap@#1\endcsname{#3}%
    \_xp\edef\csname linecap@#1@wd\endcsname{#2}%
}
...
\def\_diagarrow#1#2#3#4{%
    ...
    \fi\fi%
    \drawpath{(#1, #2).\_oxplace}{(#1, #2).\_oyplace}{(#3, #4).\_dxplace}{(#3, #4).\_dyplace}%
}
...
\def\diagarrow#1{%
    ...
    \_diagarrow{\_fromx}{\_fromy}{\_tox}{\_toy}%
}
...
%\bdrawing
%
%\addnode{$\displaystyle\sum$}{0}{0}{test}
%\addnode{$\displaystyle\sum$}{50}{0}{testing}
%
%\drawpath(test.right, test.ycenter) to (testing.left, testing.ycenter)
%
%\edrawing
%
%There
...
\drawdiagram{%
    Hello&There&My friend&Silly pup&I'm confused\cr
    $\displaystyle\sum$&Hey&There&DUDE&Me too\cr
    \cr
    Hey\cr
}{%
    \diagarrow{from={1,1}, to={2,4}}%
    %\diagarrow{from={1,3}, to={2,4}}%
    %\diagarrow{from={1,5}, to={4,1}}%
}

%\drawdiagram{%
%    \vrule height5pt width10pt&\vrule height5pt width5pt&\vrule height5pt width3pt\cr
%    \vrule height5pt width7pt&\vrule height5pt width10pt&\vrule height5pt width5pt\cr
%    \vrule height5pt width15pt&\vrule height5pt width7pt&\vrule height5pt width3pt\cr
%}{}
